...
If the repository is deployed into a web-accessible cPanel folder such as `/git`, requests must be routed into Laravel's `public/` folder. This repo includes a root redirect helper for that case, and `public/index.php` now shows a clear deployment checklist when Composer dependencies are missing instead of a blank 500 error. Full Laravel functionality still requires PHP 8.3+ and installed Composer dependencies. See `docs/cpanel-deployment.md` for the no-terminal options.
